...
class Settings(BaseSettings):
    ...
    # 상담 RAG 임베딩 왕복 타임아웃(초). 초과하면 지식 없이 답한다.
    #
    # 2026-07-28 장애: 하드코딩 1.0s로 배포했더니 VM에서 8/8 전건 초과 → 지식 주입률 0%.
    # 상담이 직무 지식 없이 답하고 있었다. 1.0s 근거는 **로컬 실측**이었는데(p50 365ms ·
    # p90 409ms) VM은 그보다 느려 상시 초과했다. 로컬 수치로 프로덕션 값을 정한 게 원인이다.
    #
    # 그래서 두 가지를 바꾼다.
    #   ① 기본값을 되돌린다(2.0) — 1.0 이전에는 RAG가 살아 있었다.
    #   ② 설정으로 뺀다 — 환경마다 지연이 다르므로 코드 배포 없이 .env로 맞춰야 한다.
    #
    # 값을 다시 정할 땐 반드시 **그 환경에서** [RAG-EMBED] 로그의 분포를 보고 정한다.
    # 타임아웃 로그는 컷 값만 찍혀서 실제 지연을 알 수 없었던 것도 이번 장애의 한 원인이다.
    rag_embed_timeout_s: float = 2.0
