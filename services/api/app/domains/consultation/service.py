...
RAG_TOP_K = 5
# 3 → 5 (0728 실측, 골든셋 40건). scope_chunks가 결과를 단일 직무로 붕괴시키는데, 그 직무의
# 청크는 단계 5개(업무요청 이해/자료·현황 확인/처리·제작·응대/검수·판단/보고·인계)라 3개만
# 주입하면 정답 단계가 밀려 빠진다. 실패는 전부 이 유형이었고 직무 오염은 0이었다.
#   추천 스코프 적중 시  정답 근거 포함  95.0% → 100.0%
#   추천 전(전역)        정답 근거 포함  90.0% →  95.0% (남은 5%는 '지배 직무 없음' 2건)
# 후보 수를 6→10으로 넓히는 건 효과가 없었다(둘 다 동일) — 병목은 후보가 아니라 주입 수다.
# 주입 청크는 ~215자라 5개로 늘려도 ~1075자다. 시뮬레이션 NPC의 RAG_TOP_K=3은 그대로 둔다
# (NPC는 아는 만큼만 말해야 하고, 이 골든셋은 상담 질문으로만 이뤄져 근거가 안 된다).
RAG_SCOPE_CANDIDATES = 6  # 스코프 판단용으로 넓게 뽑는 후보 수(직무 분포 보고 RAG_TOP_K로 좁힘)
RAG_MAX_DISTANCE = 0.31
# 재측정(0728, 골든셋 40건·코퍼스 556청크)으로 0.35에서 내렸다. 옛 근거였던
# "정답매칭 0.22~0.33"은 doc_chunks 25건 시절 값이라 더 이상 기준이 아니다.
#   in-domain 1위 max 0.267 / 오프토픽 1위 min 0.372 → 겹침 0 안전밴드 [0.267, 0.372]
#   0.30·0.35 모두 Recall@5 1.000 · 오프토픽 오통과 0% (0.40부터 20%, 0.45는 75%)
# 0.35는 밴드 상단에 붙어 거절 여유가 0.022뿐이라 0.31로 옮겼다(거절 0.062 / 회수 0.043).
# 컷은 in-domain 질의엔 사실상 무력하다 — 상위 200청크 거리 최댓값이 0.293이라 아무것도
# 못 자른다. 실제 역할은 오프토픽 거절이다.
# 이 컷으로 기록에 남은 동일계열 오염 사례(ys-01 질문에 ys-02 지식, dist 0.31)도 걸러진다
# (조건이 distance < max_distance라 0.31은 제외). 다만 같은 F 안의 인접 직무 오염은
# 거리컷으로 못 막는다 — J 청크 515개 중 490개(95%)가 같은 F 안에서 본문이 같기 때문이다.
...
# 2.0 → 1.0 (2026-07-28 오전) → 3.0 (2026-07-28 저녁, 되돌림+상향).
#
# ⚠️ 1.0s는 **단독 측정으로 정한 값이라 틀렸다.** 실서버에서 RAG를 완전히 죽였다:
#   VM 실측(상담 11턴) — RAG 실행 8건 중 **8건 전부 타임아웃 → 지식 주입률 0%**.
#   같은 VM에서 임베딩만 단독으로 재면 p50 606ms · max 1,320ms · 1초 초과 2/10으로 멀쩡하다.
#   즉 임베딩 API가 느린 게 아니라 **요청 처리 중에는 단독 조건보다 크게 느려진다.**
#   단독 벤치로 프로덕션 컷을 정하면 안 된다는 게 이 사건의 교훈이다.
#
# 앞선 주석의 "이벤트 루프 경합이 아니다"라는 결론도 로컬 단독 실험에 근거한 것이라
# VM 실사용에는 적용되지 않았다. 경합 후보가 실제로 관측된다 — 같은 시간대 로그에
# MuseTalk WS 릴레이 실패가 6건 쌓여 있고(ngrok 터널 다운), warmup은 90s 타임아웃을 건다.
#
# 3.0s 근거: 단독 max 1,320ms의 2.3배. 요청 중 열화 폭을 아직 못 재서 보수적으로 잡았다.
# 컷을 올리면 타임아웃 턴의 대기가 길어지지만, 지식 0건으로 답하는 것보다는 낫다 —
# 지식 주입률 0%는 RAG 파이프라인 전체가 무의미해진다는 뜻이다.
# 값을 바꿔 실험하려면 재배포 없이 .env의 RAG_EMBED_TIMEOUT_S로 덮어쓸 수 있다.
